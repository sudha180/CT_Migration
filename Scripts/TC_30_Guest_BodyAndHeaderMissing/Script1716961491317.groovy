...
//assert WS.verifyElementPropertyValue(GuestResponse, 'title', 'Bad Request')
...
if ((WS.getResponseStatusCode(GuestResponse) == 400) && Error_Message.equalsIgnoreCase('Unauthorized Request - Could not validate request.')) {
    println(' Testcase got passed for the missing deviceid and empty body for guest user')
} else {
    println(' Testcase failed for the missing device id and empty body for a guest user')
}
